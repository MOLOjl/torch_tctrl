...
class IntraStageDPQuery:
    ...
    def get_intra_stage(self, stage_idx, layer_start, layer_end):
        ...
        if key in self.cached_result:
            return self.cached_result[key], self.cached_solution[key], self.cached_memory[key]
        ...
        left_memory -= hidden_state * 1.5 * layer_num * (self.config.pp - stage_idx)
        ...
        if test_compute_time > self.ref_compute_time:
            print(f"test compute time: {test_compute_time}; ref compute time: {self.ref_compute_time}")
            return None, None, None

        # beibao wenti
        left_memory = int(left_memory)
        if left_memory < 0:
            print(f"left_memory: {left_memory}")
            return None, None, None
        ...
        weight_memory += 1.5 * hidden_state * (self.config.pp - stage_idx)
        if recompute_op_dict is None:
            weight_memory = -1
        else:
            for op, value in self.cost_map.items():
                weight_memory += value[0] * self.divisor * recompute_op_dict[op] * (self.config.pp - stage_idx)
        self.cached_memory[key] = weight_memory

        return record_time, recompute_op_dict, weight_memory
...
def stage_dp_algorithm(config:Config, intra_stage_dp_query: IntraStageDPQuery, output_filename):
    ...
    for i in range(layer_num - 1, -1, -1):
        result, _, _ = intra_stage_dp_query.get_intra_stage(stage_idx, i, layer_num - 1)
        if result is None:
            break
        ...
        for l_s in range(layer_num - (config.pp - s_n), -1, -1):
            for l_m in range(l_s, layer_num - (config.pp - s_n) + 1):
                ...
                if next_item is None:
                    continue
                mid_item, _, _ = intra_stage_dp_query.get_intra_stage(s_n, l_s, l_m)
                if mid_item is None:
                    break
    ...
    while stage_idx < config.pp:
        ...
        stage_time, stage_solution, _ = intra_stage_dp_query.get_intra_stage(stage_idx, layer_idx, next_layer_idx)
...
def print_elapsed_time(config:Config,
    ...
    stage_time, _, _ = intra_stage_dp_query.get_intra_stage(config.pp - 1, layer_range[0], layer_range[1])
    ...
    for stage_idx in range(config.pp - 2, -1, -1):
        ...
        mid_item, _, stage_memory = intra_stage_dp_query.get_intra_stage(stage_idx, layer_range[0], layer_range[1])
        print(f"stage[{stage_idx}]: time: {mid_item}, stage_memory: {stage_memory}")
...
def even_part_dp_algorithm(config:Config,
    ...
    for stage_idx in range(config.pp):
        ...
        stage_time, stage_solution, _ = intra_stage_dp_query.get_intra_stage(stage_idx, layer_range[0], layer_range[1])
        print(f"stage idx: {stage_idx}, layer: {layer_range}, stage time: {stage_time}")
        stage_solution["stage_idx"] = stage_idx
...
if len(sys.argv) != 8:
    print("please input world_size, tp, pp, gbs, seq_len, mem, whether even part ")
...
head_num = int(sys.argv[9])
even = int(sys.argv[10])
dp = world_size // tp // pp

with open(f"recompute_config/gpt/profile_result_{tp}tp_{pp}pp_{seq_len}seq_{hidden_size}hidden.json", 'r') as f:
...
cost_map_mp4 = {
    ...
    OperatorType.attn_key: (hidden, profile_dict[OperatorType.attn_key]),
    OperatorType.attn_value: (hidden, profile_dict[OperatorType.attn_value]),
    OperatorType.attn_fa: (hidden, profile_dict[OperatorType.attn_fa]),
    ...
    OperatorType.linear_mlp1: (mapping0, profile_dict[OperatorType.linear_mlp1]),
    OperatorType.linear_activation: (mapping0, profile_dict[OperatorType.linear_activation]),
}

max_divisor = gcd(mapping0, hidden)
for op, (gmem, cost) in cost_map_mp4.items():
    cost_map_mp4[op] = (gmem // max_divisor, cost)
...
if even <= 0:
    output_filename = f"recompute_config/gpt/adapipe/gpt_{gpt_config.seq_len}seq_{hidden_size}hidden_{layer_num}ln_{gpt_config.mp}mp_{gpt_config.pp}pp_{gbs}gbs_{mem}mem.json"
    stage_dp_algorithm(gpt_config, intra_stage_dp_query, output_filename)
else:
    layer_per_stage = gpt_config.num_layers // gpt_config.pp
    output_filename = f"recompute_config/gpt/evenpart/gpt_{gpt_config.seq_len}seq_{hidden_size}hidden_{layer_num}ln_{gpt_config.mp}mp_{gpt_config.pp}pp_{mem}mem.json"
    stage_list = []
