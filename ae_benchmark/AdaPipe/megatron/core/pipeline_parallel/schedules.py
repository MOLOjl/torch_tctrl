...
def backward_step(input_tensor, output_tensor, output_tensor_grad, model_type, config):
    ...
    args = get_args()
    if args.profile_recompute and (parallel_state.get_pipeline_model_parallel_rank() == 0):
        torch.cuda.synchronize()
        logging.info(f"before embedding backward: {time.time()}")
    ...
    *,
    ...
    for i in range(num_warmup_microbatches):
        ...
        if args.profile_memory:
            torch.cuda.synchronize()
            logging.info(f"timestamp: {time.time()}, before micro step: {i} forward, memory: {torch.cuda.memory_allocated() / 1024 / 1024} MB, reserved: {torch.cuda.memory_reserved() / 1024 / 1024} MB")
        ...
        if args.profile_memory:
            torch.cuda.synchronize()
            logging.info(f"timestamp: {time.time()}, after micro step: {i} forward, memory: {torch.cuda.memory_allocated() / 1024 / 1024} MB, reserved: {torch.cuda.memory_reserved() / 1024 / 1024} MB")
            #if torch.distributed.get_rank() == 0:
            #    torch.cuda.memory._dump_snapshot(f"memory_pickle/micro_step_{i}_forward.pickle")
    ...
    for i in range(num_microbatches_remaining):
        ...
        if args.profile_memory:
            torch.cuda.synchronize()
            logging.info(f"timestamp: {time.time()}, before micro step: {i + num_warmup_microbatches} forward, "
                         f"memory: {torch.cuda.memory_allocated() / 1024 / 1024} MB, "
        ...
        if args.profile_memory:
            torch.cuda.synchronize()
            logging.info(f"timestamp: {time.time()}, after micro step: {i + num_warmup_microbatches} forward, "
                         f"memory: {torch.cuda.memory_allocated() / 1024 / 1024} MB, "
                         f"max memory: {torch.cuda.max_memory_allocated() / 1024 / 1024} MB, "
                         f"reserved: {torch.cuda.memory_reserved() / 1024 / 1024} MB")
        ...
        if forward_only:
            ...
            if args.profile_memory:
                torch.cuda.synchronize()
                logging.info(f"timestamp: {time.time()}, before micro step: {i + num_warmup_microbatches} backward, "
                             f"memory: {torch.cuda.memory_allocated() / 1024 / 1024} MB, "
                             f"max memory: {torch.cuda.max_memory_allocated() / 1024 / 1024} MB, "
                             f"reserved: {torch.cuda.memory_reserved() / 1024 / 1024} MB")
            input_tensor_grad = backward_step(
            ...
            if args.profile_memory:
                torch.cuda.synchronize()
                logging.info(f"timestamp: {time.time()}, after micro step: {i + num_warmup_microbatches} backward, "
                             f"memory: {torch.cuda.memory_allocated() / 1024 / 1024} MB, "
                             f"max memory: {torch.cuda.max_memory_allocated() / 1024 / 1024} MB, "
                             f"reserved: {torch.cuda.memory_reserved() / 1024 / 1024} MB")
